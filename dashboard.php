...
$sql = "SELECT id_pasien, username, nama_pasien, email, no_telepon FROM pasien WHERE username = ?";
$stmt = $db->prepare($sql);
...
// Ambil semua appointment milik pasien ini
$sql = "SELECT a.*, d.nama_dokter, d.spesialisasi, d.biaya_konsultasi, p.metode_bayar 
        FROM appointment a
...
$stmt = $db->prepare($sql);
$stmt->bind_param("i", $user['id_pasien']);
$stmt->execute();
...
<!-- Tambahan pada bagian bawah konten dashboard -->
<div class="bg-white rounded-xl p-6 smooth-shadow card-hover">
    <div class="flex items-center mb-6">
        <div class="bg-primary-100 p-3 rounded-full mr-4">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <h2 class="text-2xl font-bold text-secondary-800">Riwayat Booking Konsultasi</h2>
    </div>
    <?php if ($appointments && $appointments->num_rows > 0): ?>
        <div class="flex flex-col space-y-4">
            <?php while($row = $appointments->fetch_assoc()): ?>
                <div class="flex flex-col sm:flex-row sm:items-start bg-secondary-50 rounded-xl p-4 shadow-md">
                    <div class="flex-shrink-0 bg-blue-100 text-blue-600 p-3 rounded-lg mb-2 sm:mb-0">
                    ...
                    <div class="ml-0 sm:ml-4 flex-grow">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                            ...
                            <span class="text-sm mt-1 sm:mt-0 px-3 py-1 rounded-full <?= $row['status'] == 1 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' ?> font-medium">
                                <?= $row['status'] == 1 ? 'Selesai' : 'Belum Dibayar' ?>
                            </span>
                        </div>
                        <p class="text-sm text-secondary-700">Spesialis: <?= htmlspecialchars($row['spesialisasi']) ?></p>
                        <p class="text-sm text-secondary-600">
                           <p class="text-secondary-700">
  Jadwal: <?= date('d M Y', strtotime($row['waktu'])) . ' pukul ' . date('H:i', strtotime($row['jam'] ?? '00:00:00')) ?>
</p>

                        <p class="text-sm text-secondary-600">
                            Biaya Konsultasi: Rp <?= number_format($row['biaya_konsultasi'], 0, ',', '.') ?>
                        </p>
                        <p class="text-sm text-secondary-600">
                            Metode Pembayaran: <?php 
                                if (isset($row['metode_bayar'])) {
                                    switch($row['metode_bayar']) {
                                        case 1: echo 'Transfer Bank'; break;
                                        case 2: echo 'E-Wallet'; break;
                                        case 3: echo 'Kartu Kredit/Debit'; break;
    ...
    <?php else: ?>
        <p class="text-secondary-600">Belum ada data booking.</p>
    <?php endif; ?>
</div>
                <!-- Health Status -->
                <div class="bg-white rounded-xl p-6 smooth-shadow card-hover">
