...
if (isset($_POST['book_appointment'])) {
    ...
    $waktu = $_POST['waktu'];
    $jam = $_POST['jam_konsultasi'];

    try {
        $sql = "INSERT INTO appointment (id_pasien, id_dokter, waktu, jam) VALUES (?, ?, ?, ?)";
        $stmt = $db->prepare($sql);
        $stmt->bind_param("iiss", $pasien_id, $dokter_id, $waktu, $jam);
...
<body class="bg-secondary-100 min-h-screen flex flex-col font-sans antialiased">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="bg-white rounded-xl p-8 smooth-shadow animate__animated animate__fadeIn animate__delay-1s">
                <form method="POST" action="booking.php" class="space-y-6">
                    ...
                    <div>
                        <label for="jam_konsultasi" class="block text-sm font-medium text-secondary-700 mb-2">Pilih Jam Konsultasi</label>
                        <select name="jam_konsultasi" id="jam_konsultasi" required 
                                class="form-input w-full px-4 py-3 rounded-lg focus:ring-primary-500 focus:border-primary-500">
                            <option value="">Pilih tanggal & dokter terlebih dahulu</option>
                        </select>
                    </div>
    ...
    <!-- AJAX untuk ambil jam praktik -->
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const dokterSelect = document.getElementById('id_dokter');
            const tanggalInput = document.getElementById('waktu');
            const jamSelect = document.getElementById('jam_konsultasi');
            ...
            function fetchJam() {
                ...
                fetch(`get_jadwal.php?id_dokter=${dokterId}&tanggal=${tanggal}`)
                    .then(response => response.json())
                    .then(data => {
                        jamSelect.innerHTML = '';
                        if (data.length === 0) {
                            jamSelect.innerHTML = '<option value="">Tidak ada jadwal tersedia</option>';
                        } else {
                            data.forEach(jam => {
            ...
            dokterSelect.addEventListener('change', fetchJam);
            tanggalInput.addEventListener('change', fetchJam);
        });
    </script>
</body>
</html>
